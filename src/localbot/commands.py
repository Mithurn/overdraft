...
PREVHANDOFF_COMMAND = """\
---
description: Export this session and restart on CloudAI free backends with a handoff summary
allowed-tools: Bash(cloudai:*)
---

The user wants to move this session to CloudAI free backends with a clean restart.

1. Tell the user you are handing off and they should run this in their terminal if it does not start automatically:
   `cloudai prevhandoff`
2. If Bash is available, run `cloudai prevhandoff` yourself.
